...
const updateImageStatus = async (req,res) => {
    ...
    try{
        const updatedImage = await doctorImageModel.findByIdAndUpdate(id,{status: status},{new: true});
...
const getImageById = async (req,res) => {
    ...
    try{
        const image = await doctorImageModel.findById(id)
...
const getImagesByHospital = async (req,res) => {
    ...
    try {
        const images = await doctorImageModel.find({ doctor: id });
...
const deleteImage = async (req,res) => {
    ...
    try{
        const deletedImage = await doctorImageModel.findByIdAndDelete(id);
